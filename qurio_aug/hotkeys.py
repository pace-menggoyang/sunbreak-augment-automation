...
Defaults to Control(⌃)+M (start) / Control(⌃)+N (stop) rather than
function keys: many Mac keyboards map F-keys to brightness/volume/Mission
Control by default, needing an extra Fn press to get the literal F-key --
not "reachable" at all. Control+letter needs no Fn and is one comfortable
one-handed reach.

M/N specifically (not, say, S/X) because pynput's macOS listener observes
key events globally but doesn't consume/suppress them -- the raw keydown
still reaches whichever app is focused underneath it. Confirmed live:
Option+S/Option+X leaked through as plain S/X to the game (which doesn't
check modifier state for its own bindings), triggering in-game
...
Control, not Option, as the modifier: Option is macOS's system-wide
modifier for typing accented/special characters (Option+M literally types
"µ"), and pynput resolves keys on macOS by the *character* an event
produces after that substitution -- so Option+M showed up to our listener
as "µ", not "m", and never matched the registered "<alt>+m" combo at all
(confirmed live: it printed "µ" into the terminal instead of firing).
Control doesn't participate in that substitution table, so Control+letter
resolves to the plain letter and matches reliably.
"""
